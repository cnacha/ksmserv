...
public class CalendarAPITester {
	...
	private final String ACCOUNT_ID = "ksm-smartqueue@appspot.gserviceaccount.com";


	public CalendarAPITester() throws Exception {
		credentials = new GoogleCredential.Builder().setTransport(GoogleNetHttpTransport.newTrustedTransport())
				...
				.setServiceAccountPrivateKeyFromP12File(new File("WEB-INF/ksm-smartqueue-bc7b46f01ae8.p12")).build();
	...
	public boolean insertCalendar(String calendarName){
		 Calendar entry = new Calendar();
		 entry.setSummary(calendarName);
		
		 try {
			if(client.calendars().insert(entry).execute() != null){
				 return true;
			 }
		} catch (IOException e) {
